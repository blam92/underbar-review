(function() {
  ...
  _.last = function(array, n) {
    return n === undefined ? array[array.length - 1] : array.length < n ? array.slice() : array.slice(array.length - n);
  };
  ...
  _.uniq = function(array, isSorted, iterator) {
    ...
    if (isSorted) {
      ...
      for (var i = 0; i < array.length; i++) {
        ...
        if (i === 0) {
          result.push(array[i]);
        } else if (iteratedValue !== iterator(array[i - 1])) {
          result.push(array[i]);
        }
      }
    } else {
      for (var i = 0; i < array.length; i++) {
        var iteratedValue = iterator(array[i]);
        if (uniq[iteratedValue] === undefined) {
          uniq[iteratedValue] = array[i];                                                    
        }
      }
      for (var key in uniq) {
